...
//    });
    ...
    public static Expr toExpr(Val arg, CompEnv toExpr) {
        return ((Declared) arg._UNPACK_()).toExpr(toExpr);
    }
    ...
    private static final class Declared {
        ...
        public Expr toExpr(CompEnv toExpr) {
            if (key.getType() == SymbolFw.symbol) {
                return ExprList.of(BracketsTypes.round, Symbol.of(":"), (Symbol) ExprFw.unwrap(key), toExpr.toExpr(value));
            }
            return ExprList.of(BracketsTypes.round, Symbol.of("Declared"), toExpr.toExpr(key), toExpr.toExpr(value));
        }
            ...
            if (expr instanceof ExprList && ((ExprList) expr).getBracketsType().equals(BracketsTypes.round) && ((ExprList) expr).size() > 0) {
                ...
                if (f instanceof Symbol) switch (((Symbol) f).getValue()) {
                    case ":": {
                        ...
                        return VitFw.wrap(Vit.val(declared.asVal()).call(symbol("builder")).call(name).call((Vit) value._UNPACK_()));
                    }
                }
            }
        }
